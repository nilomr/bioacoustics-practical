...
>  - Are our spectral parameters the best suited for the job?<br>
...
We will now train a relatively simple machine learning algorithm called a Random Forest to see if we can predict which species sung any given note in our dataset --- using the same features that we extracted earlier.
...
What can you say about the model's performance?
To help visualise it we can plot a confusion matrix, where you can see the number of true positives and negatives for each species when we tested our model on the data we hid from it during training:

> **Question**:<br>
>  - Why can't we just train the algorithm using all available data?<br>
